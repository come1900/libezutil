 ...
 * d3des.c - _explain_
 *
 * $Id: d3des.c 5884 2008-7-16 11:33:42Z abc $
 *
 *  Explain:
 *     d3des
 *
...
#define EZ_DES_BLOCK_KEY_SIZE 8

int __ez_des_set_key(const char *pKey,
                     ...
                     unsigned int iFixedKeyLen)
{
	if (NULL == pKey
	    ||NULL == pFixedKey
	    || iFixedKeyLen<EZ_DES_BLOCK_KEY_SIZE)
	{
		return  -1;
	}

	memset(pFixedKey, 0, iFixedKeyLen);
	memcpy(pFixedKey, pKey, iKeyLen>EZ_DES_BLOCK_KEY_SIZE?EZ_DES_BLOCK_KEY_SIZE:iKeyLen);
                    ...
                    int iKeylen)
{
	unsigned int i;
	unsigned int j;
	unsigned char desKey[EZ_DES_BLOCK_KEY_SIZE];

	unsigned char laststr[EZ_DES_BLOCK_KEY_SIZE];

	if( !( pResult && pOrig && pKey ))
		return -1;

	__ez_des_set_key(pKey, iKeylen, desKey, EZ_DES_BLOCK_KEY_SIZE);

	deskey(desKey, EN0);

	j=(unsigned int)(iOrigLen/(double)8);
	for(i=0; i<j; i++,pResult+=8,pOrig+=8)
	{
		des(pOrig, pResult);
	}
	...
	{
		// filled 0
		memset(laststr, 0, EZ_DES_BLOCK_KEY_SIZE);
		memcpy(laststr, pOrig, iOrigLen%8);
		des(laststr, pResult);
	}
                    ...
                    int iKeylen)
{
	...
	unsigned char desKey[EZ_DES_BLOCK_KEY_SIZE];
	 ...
	 if( !( pResult && pOrig && pKey && (iOrigLen%8==0) ) )
		return -1;

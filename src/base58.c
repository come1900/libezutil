 ...
 *
 * $Id: base58.c 5884 2023-05-01 03:49:26Z WuJunjie $
 *
 *  Explain:
 *     -explain-
 *
 *  Update:
 *     2023-05-01 03:49:26  Create
 */
/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-*/
#include <limits.h>
...
int base58_encode(const unsigned char *in, size_t in_len, unsigned char *out, size_t *out_len) {
    if (!in_len) {
        *out_len = 0;
        return 0;
    }
...
int base58_decode(const unsigned char *in, size_t in_len, unsigned char *out, size_t *out_len) {
    if (!in_len) {
        *out_len = 0;
        return 0;
    }
...
size_t base58_encode_length(size_t in_len)
{
	return (size_t)(in_len * 1.37 + 1);
}

size_t base58_decode_length(size_t in_len)
{
	return (size_t)((double)in_len/1.37+0.5);
    // Len58 = Len256 * ( log256 / log58 ) + 1
    // Len58-1/ = Len256 * ( log256 / log58 ) + 1
}
